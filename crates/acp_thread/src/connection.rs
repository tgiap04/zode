...
/// Upstream also carried a `provider_id: Option<LanguageModelProviderId>` here, to
/// route authentication at one of Zed's own model providers. External agents hold
/// their own credentials — `claude` and `codex` each sign in through their CLI —
/// so that field described a path this fork does not have, and dropping it is what
/// keeps `language_model` out of the dependency graph.
