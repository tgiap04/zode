...
/// One line of the list: a group header, or a session under it.
...
///    different roots.
/// 2. **Group by the session's own `cwd`.** Never by the encoded directory name
///    Claude stores them under: `/a/b-c` and `/a-b-c` both encode to `-a-b-c`, so
///    that name cannot be decoded back into a path.
/// 3. **Newest first**, both within a group and between groups — a group is as
///    recent as its most recent session.
///
...
pub(crate) fn rows(
    ...
    let mut groups: Vec<(PathBuf, Vec<SessionSummary>)> = Vec::new();
    for session in sessions {
        ...
        match groups.iter_mut().find(|(cwd, _)| *cwd == session.cwd) {
            ...
            None => groups.push((session.cwd.clone(), vec![session.clone()])),
        }
    }

    // `sessions` arrives newest first, so each group's first entry is its most
    // recent one and the group order follows from it.
    let mut out = Vec::new();
    for (cwd, sessions) in groups {
        let collapsed = collapsed.contains(&cwd);
        out.push(Row::Group {
...
impl Render for AgentHistoryPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let rows = rows(&self.sessions, &roots, &query, &self.collapsed_groups);
                ...
                gpui::list(
                    ...
                    cx.processor(move |this, ix: usize, _window, cx| match &rows[ix] {
                        Row::Group {
                            ...
                            count,
                            collapsed,
                        } => row::render_group(ix, path, label, *count, *collapsed, cx)
                            .into_any_element(),
                        Row::Session(session) => {
...
mod tests {
    ...
    fn only_sessions_of_the_open_project_are_listed() {
        ...
        let listed = rows(&all(), &roots, "", &HashSet::default());
    ...
    fn with_no_project_open_nothing_is_listed() {
        assert!(rows(&all(), &[], "", &HashSet::default()).is_empty());
    }

    #[test]
    fn groups_carry_a_label_and_a_count() {
        let roots = vec![PathBuf::from("/w/zode")];
        let rows = rows(&all(), &roots, "", &HashSet::default());
    ...
    fn a_collapsed_group_keeps_its_header_and_drops_its_sessions() {
        ...
        collapsed.insert(PathBuf::from("/w/zode"));
        let rows = rows(&all(), &roots, "", &collapsed);
        assert_eq!(rows.len(), 3, "two headers and the one uncollapsed session");
        assert!(matches!(
            &rows[0],
            Row::Group {
                collapsed: true,
    ...
    fn the_query_matches_every_field_the_row_shows() {
        let roots = vec![PathBuf::from("/w/zode")];
        let hits = |query: &str| {
            rows(&all(), &roots, query, &HashSet::default())
                .iter()
