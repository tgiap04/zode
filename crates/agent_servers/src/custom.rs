...
impl AgentServer for CustomAgentServer {
    ...
    fn connect(
        ...
        cx.spawn(async move |cx| {
            // Upstream also fetched a Gemini API key from Zed's own keychain here.
            // That reached through `language_model::ApiKey` and `google_ai`, neither
            // of which this fork carries, and it only served an agent zode does not
            // ship. Gemini still works if the user exports `GEMINI_API_KEY` — the
