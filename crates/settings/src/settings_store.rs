...
mod tests {
    ...
    struct GlobalOnlySetting {
        vim_mode: bool,
    }

    impl Settings for GlobalOnlySetting {
        fn from_settings(content: &SettingsContent) -> Self {
            GlobalOnlySetting {
    ...
    fn test_settings_store_basic(cx: &mut App) {
        ...
        assert_eq!(
            store.get::<GlobalOnlySetting>(None),
            &GlobalOnlySetting { vim_mode: false }
        );
        assert_eq!(
        ...
        assert_eq!(
            store.get::<GlobalOnlySetting>(None),
            &GlobalOnlySetting { vim_mode: true }
        );
        assert_eq!(
            store.get::<ItemSettings>(None).close_position,
            ClosePosition::Left
        ...
        assert_eq!(
            store.get::<GlobalOnlySetting>(Some(SettingsLocation {
                worktree_id: WorktreeId::from_usize(1),
                path: rel_path("root2/something")
            })),
            &GlobalOnlySetting { vim_mode: true }
    ...
    fn test_setting_store_update(cx: &mut App) {
        ...
        check_settings_update(
            ...
            r#"{ "vim_mode": true, "one": 1, "two": 2 }"#.to_owned(),
            cx,
        );
