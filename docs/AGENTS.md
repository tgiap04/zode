...
| ----------------------------- | ------------------------------------- | ------------------------------- |
| `{#kb action::ActionName}`    | Keybinding for action                 | `{#kb project_panel::ToggleFocus}` |
| `{#action agent::ActionName}` | Action reference (renders as command) | `{#action zed::OpenSettings}`      |
