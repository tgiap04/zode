...
- **What identifies "a project" when per-project state (such as a dock's remembered size) is involved?**: it is the on-disk path, taken as plain text. Two pre-existing ambiguities in that identity became visible now that state hangs off it: on a case-insensitive filesystem, `/Users/me/Repo` and `/Users/me/repo` are counted as two different projects even though they're one directory, and a checkout reached through two different symlinks is likewise counted twice. Neither is new to this feature — a developer could already reach the same directory two ways — but a user may now notice it as "my dock size didn't carry over," where before there was no state to disagree about.
