...
| Project rail (always-visible sidebar) | A narrow, always-visible column of project squares (initials/icon), one per open project, with a muted clock icon on hibernated entries and a warning icon while re-indexing (`crates/sidebar/src/rail.rs`, `project_item.rs`) | Click a project to switch to it; use `NextProject`/`PreviousProject` to cycle; toggle the wider sidebar panel open/closed |
