...
| Project rail (always-visible sidebar)          | A narrow, always-visible column of project squares (a custom logo image if one is set, otherwise initials/icon), one per open project, with a muted clock icon on hibernated entries and a warning icon while re-indexing; below them, a fixed block of left-dock panel icons acting as the VS Code-style activity bar (`crates/sidebar/src/rail.rs`, `rail_panels.rs`, `project_item.rs`, `rail_avatar.rs`) | Click a project to switch to it; use `NextProject`/`PreviousProject` to cycle; right-click a square for a context menu (open in new window, reveal in Finder, copy path, change initials/colour/logo, remove logo, remove project); click a panel icon to bring that panel forward, or click the active one to hide the dock; toggle the wider sidebar panel open/closed |
| Sidebar panel (expanded)                       | The same project list with full names/paths and a filter input, plus per-project context menu; a project's row shows its custom logo next to the name once one is set (no placeholder square when there isn't one)                                                                                                                          | Filter/search open projects; access per-project actions (e.g. move to new window) via the ellipsis menu                                                                                                             |
