...
- Click the Claude Code, Codex, Antigravity, Copilot, or opencode icon in the sidebar rail to open that agent — it runs as a terminal session of the agent's own CLI, in a tab beside your files.
- Each agent needs its own CLI installed: `claude`, `codex`, `agy`, `copilot`, and `opencode` respectively. Clicking an agent whose CLI is missing offers you the vendor's install command rather than failing silently.
- To start a second session of an agent that's already open, use the `+` menu in the tab bar and pick "New Claude Code", "New Codex", "New Antigravity", "New Copilot", or "New opencode".

Two things are uneven across the five, because the CLIs themselves are uneven:

- **Session history** covers Claude Code, Codex, Copilot and opencode, each of which writes its conversations somewhere on this machine. Antigravity does not — `agy` keeps its conversations server-side and leaves nothing on disk, so it has no rows in the history panel.
- **Subscription quota** on the status bar covers Claude Code and Codex only. Antigravity and Copilot publish no way to read it that doesn't require this editor to hold a credential of its own; opencode is bring-your-own-key and has no subscription quota to publish in the first place. See [Agent Usage Indicator](./agent-usage.md) for both.
