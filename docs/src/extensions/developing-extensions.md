...
description: "Create Zode extensions: languages, themes, debuggers, and more."
---

# Developing Extensions {#developing-extensions}

Zode extensions are Git repositories containing an `extension.toml` manifest. They can provide languages, themes, debuggers, snippets, and MCP servers.
...
Before starting to develop an extension for Zode, be sure to [install Rust via rustup](https://www.rust-lang.org/tools/install).
...
When developing an extension, you can use it in Zode without needing to publish it by installing it as a _dev extension_.
...
If you need to troubleshoot, check Zode.log ({#action zed::OpenLog}) for additional output. For debug output, close and relaunch Zode from the command line with `zed --foreground`, which shows more verbose INFO-level logs.
...
## Directory Structure of a Zode Extension

A Zode extension is a Git repository that contains an `extension.toml`. This file must contain some
...
In addition to this, there are several other optional files and directories that can be used to add functionality to a Zode extension. An example directory structure of an extension that provides all capabilities is as follows:
...
Use the latest version of the [`zed_extension_api`](https://crates.io/crates/zed_extension_api) available on crates.io. Make sure it's still [compatible with Zode versions](https://github.com/zed-industries/zed/blob/main/crates/extension_api#compatible-zed-versions) you want to support.
...
> `stdout`/`stderr` is forwarded directly to the Zode process. In order to see `println!`/`dbg!` output from your extension, you can start Zode in your terminal with a `--foreground` flag.

## Forking and cloning the repo

1. Fork the repo

> **Note:** It is very helpful if you fork the `zed-industries/extensions` repo to a personal GitHub account instead of a GitHub organization, as this allows Zode staff to push any needed changes to your PR to expedite the publishing process.
...
- Extension IDs and names must not contain the words `zed`, `Zed` or `extension`, since they are all Zode extensions.
...
- Extensions should provide something that is not yet available in the marketplace as opposed to fixing something that could be resolved within an existing extension. For example, if you find that an existing extension's support for a language server is not functioning properly, first try contributing a fix to the existing extension as opposed to submitting a new extension immediately.
  - If you receive no response or reaction within the upstream repository within a reasonable amount of time, feel free to submit a pull request that aims to fix said issue. Please ensure that you provide your previous efforts within the pull request to the extensions repository for adding your extension. Zode maintainers will then decide on how to proceed on a case by case basis.
- Extensions that intend to provide a language, debugger or MCP server must not ship the language server as part of the extension. Instead, the extension should either download the language server or check for the availability of the language server in the users environment using the APIs as provided by the [Zode Rust Extension API](https://docs.rs/zed_extension_api/latest/zed_extension_api/).
...
Once your PR is merged, the extension will be packaged and published to the Zode extension registry.
