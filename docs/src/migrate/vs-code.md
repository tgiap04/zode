---
title: How to Migrate from VS Code to Zode
description: "Guide for migrating from VS Code to Zode, including settings and keybindings."
---

# How to Migrate from VS Code to Zode

This guide explains how to move from VS Code to Zode without rebuilding your workflow.
...
After installation, you can launch Zode from your Applications folder (macOS) or directly from the terminal (Linux) using:
`zed .`
This opens the current directory in Zode.

## Import Settings from VS Code

During setup, you have the option to import key settings from VS Code. Zode imports the following settings:
...
Zode doesn’t import extensions or keybindings, but this import gets core editor behavior close to your VS Code setup. If you skip that step during setup, you can still import settings manually later via the command palette:
...
| VS Code | Zode | Notes |
| --- | --- | --- |
| editor.fontFamily | buffer_font_family | Zode uses Zed Mono by default |
...
Zode also supports per-project settings. You can find these in the Settings Editor as well.

## Open or Create a Project

After setup, press `Cmd+O` (`Ctrl+O` on Linux) to open a folder. This becomes your workspace in Zode. There's no support for multi-root workspaces or `.code-workspace` files like in VS Code. Zode keeps it simple: one folder, one workspace.
...
### Common Shared Keybindings (Zode <> VS Code)
...
Zode also supports chords (multi-key sequences) like `Cmd+K Cmd+C`, like VS Code does.
...
Zode simplifies this model.

In Zode:

- There is no workspace file format. Opening a folder is your project context.

- Zode does not support multi-root workspaces. You can only open one folder at a time in a window.
...
- Per-project settings are optional. You can add a `.zed/settings.json` file inside a project to override global settings, but Zode does not use `.code-workspace` files and won’t import them.

- You can start from a single file or an empty window. Zode doesn’t require you to open a folder to begin editing.
...
Zode takes a different approach:
...
- Instead of a persistent panel, Zode encourages you to:
...
Zode does not offer as many extensions as VS Code. The available extensions are focused on language support, themes, syntax highlighting, and other core editing enhancements.

Several features that typically require extensions in VS Code are built into Zode:
...
You won’t find one-to-one replacements for every VS Code extension, especially if you rely on tools for DevOps, containers, or test runners. Zode's extension catalog is still growing and remains smaller.

### Collaboration in Zode vs. VS Code

Unlike VS Code, Zode doesn’t require an extension to collaborate. It’s built into the core experience.
...
Learn how [Zode uses Zode](https://zed.dev/blog/zed-is-our-office) to plan work and collaborate.

### Using AI in Zode

If you’re used to GitHub Copilot in VS Code, you can do the same in Zode. You can also explore other agents through Zed Pro, or bring your own keys and connect without authentication. You can disable AI features entirely if you prefer.
...
Copy your VS Code snippet JSON directly into Zode's snippets folder (`zed: configure snippets`).
