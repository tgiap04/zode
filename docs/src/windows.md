...
- Installed via installer: Use `Settings` → `Apps` → `Installed apps`, search for Zode, and click Uninstall.
...
Zode supports remote development on Windows through both SSH and WSL. You can connect to remote servers via SSH or work with files inside WSL distributions directly from Zode.
...
### Zode fails to start or shows a blank window

- Check that your hardware and operating system version are compatible with Zode. See our [installation guide](./installation.md) for more information.
...
- Try launching Zode with no extensions or custom settings to isolate conflicts.
...
#### Zode fails to open / degraded performance

Zode requires a DirectX 11 compatible GPU to run. If Zode fails to open, your GPU may not meet the minimum requirements.
...
If you're running Zode inside a virtual machine, it will use the emulated adapter provided by your VM. While Zode will work in this environment, performance may be degraded.
