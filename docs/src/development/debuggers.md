...
description: "Guide to using a debugger for Zode development."
---

# Using a debugger

> This page is not about [configuring Zode's debugger](../debugger.md).
> It covers how to use a debugger while developing Zode itself.

## Using Zode's built-in debugger

While the Zode project is open you can open the `New Process Modal` and select the `Debug` tab. There you can see two debug configurations to debug Zode with, one for GDB and one for LLDB. Select the configuration you want and Zode will build and launch the binary.
...
To get the full debugger experience on a release build, compile a Zode binary with full debug info.
...
## Running Zode with a shell debugger GDB/LLDB

### Background

When you install Rust through rustup (the recommended setup for Zode development; see your platform guide [here](../development.md)), rustup also installs helper scripts for debugging Rust binaries.
...
After enabling full debug info and building with `cargo build`, run `rust-gdb` or `rust-lldb` against the compiled Zode binary:
...
You can also attach to a running Zode process (for example, one started with `cargo run`):
...
`<pid>` is the process ID of the Zode instance you want to attach to.
...
Debuggers are useful for finding the cause of panics and crashes, including in Zode.
...
You can find additional information on debugging Zode crashes [here](./debugging-crashes.md).
