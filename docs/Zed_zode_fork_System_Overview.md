...
The following come from an experimental, lower-confidence internal document (`design-intent.md`) that reconstructs _why_ certain choices were made, rather than _what_ was built. Unlike the items above, these are inferred from indirect evidence (commit messages and code comments) rather than confirmed facts, and should be treated as discussion points rather than settled findings:
